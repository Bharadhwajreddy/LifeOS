...
import { Plus, Trash2, Heart, Film, Gift, Calendar, Lightbulb } from 'lucide-react'
    ...
    <div className="space-y-4">
          ...
          {unwatched.map((m) => (
            <Card key={m.id} className="flex items-center gap-3 px-4 py-3.5 mb-2">
              <button
                ...
                className="w-6 h-6 rounded-full border-2 border-zinc-300 dark:border-zinc-600 shrink-0 hover:border-emerald-400 transition-colors"
              />
              ...
              <button onClick={() => deleteMovie(m.id)} className="p-1.5 text-zinc-300 hover:text-rose-400 transition-colors">
                <Trash2 size={14} />
              </button>
            </Card>
          ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Add Movie / Show">
        <div className="space-y-4">
          ...
          <Input label="Genre (optional)" value={form.genre} onChange={(e) => setForm({ ...form, genre: e.target.value })} placeholder="Sci-fi, Romance, Thriller..." />
          <Btn size="lg" onClick={() => { if (form.title) { addMovie(form); setForm({ title: '', genre: '' }); setModal(false) } }}>
            Add to List
          </Btn>
        </div>
      </Modal>
  ...
  return (
    <div className="space-y-4">
          ...
          {pending.map((g) => (
            <Card key={g.id} className="flex items-center gap-3 px-4 py-3.5 mb-2">
              <button
                ...
                className="w-6 h-6 rounded-full border-2 border-pink-300 dark:border-pink-700 shrink-0 hover:border-pink-500 transition-colors"
              />
              <div className="flex-1">
              ...
              {g.price && (
                <span className="text-sm font-bold text-zinc-400 shrink-0">{currency}{g.price}</span>
              )}
              <button onClick={() => deleteGift(g.id)} className="p-1.5 text-zinc-300 hover:text-rose-400 transition-colors">
                <Trash2 size={14} />
              </button>
            </Card>
          ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Gift Idea">
        <div className="space-y-4">
          <Input label="Gift name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Perfume, Jewellery..." />
          <Input label="Why she'll love it" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Add a personal note..." />
          ...
          <Btn size="lg" onClick={() => { if (form.name) { addGift(form); setForm({ name: '', note: '', price: '' }); setModal(false) } }}>
            Save Idea
          </Btn>
        </div>
      </Modal>
    ...
    <div className="space-y-4">
      ...
      {sorted.map((d) => {
        ...
        return (
          <Card key={d.id} className={`flex items-center gap-3 px-4 py-3.5 mb-2 ${soon ? 'ring-2 ring-pink-300 dark:ring-pink-500/40' : ''}`}>
            <div className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center shrink-0 text-white text-xs font-bold ${
              soon ? 'bg-pink-500' : past ? 'bg-zinc-300 dark:bg-zinc-700' : 'bg-zinc-200 dark:bg-zinc-800'
            }`}>
              <span className={`text-base font-bold leading-none ${past ? 'text-zinc-500' : soon ? 'text-white' : 'text-zinc-600 dark:text-zinc-300'}`}>
                {format(parseISO(d.date), 'd')}
              </span>
              <span className={`text-[9px] uppercase tracking-wide ${past ? 'text-zinc-400' : soon ? 'text-pink-100' : 'text-zinc-400 dark:text-zinc-500'}`}>
                {format(parseISO(d.date), 'MMM')}
              </span>
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">{d.name}</p>
              <p className={`text-xs mt-0.5 font-medium ${soon ? 'text-pink-500' : 'text-zinc-400'}`}>
                {daysLabel(d.date)}
              </p>
              {d.note && <p className="text-xs text-zinc-400 mt-0.5">{d.note}</p>}
            </div>
            <button onClick={() => deleteDate(d.id)} className="p-1.5 text-zinc-300 hover:text-rose-400 transition-colors">
              <Trash2 size={14} />
            </button>
          </Card>
        )
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Important Date">
        <div className="space-y-4">
          ...
          <Btn size="lg" onClick={() => { if (form.name && form.date) { addDate(form); setForm({ name: '', date: '', note: '' }); setModal(false) } }}>
            Save Date
          </Btn>
        </div>
      </Modal>
          ...
          {pending.map((d) => (
            <Card key={d.id} className="flex items-center gap-3 px-4 py-3.5 mb-2">
              <button
                ...
                className="w-6 h-6 rounded-full border-2 border-pink-300 dark:border-pink-600 shrink-0 hover:border-pink-500 transition-colors"
              />
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2.5">
        <div className="w-9 h-9 rounded-2xl bg-pink-500 flex items-center justify-center shadow-sm shadow-pink-500/30">
